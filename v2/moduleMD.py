...
def vInit(pos,dist='constant',val=0,nondim='no'):
    ...
    ## Custom infile
        ...
        with open(val,'r') as fid:
            ...
            vels = np.append(tmp,vels,axis=1) #append types to vels
            types = utypes.reshape(n,1)
            vels = np.append(np.arange(1,num+1).reshape(num,1),vels,axis=1) 
            #particle ids
    
    else: 
        sys.exit('\n\tEnter a valid velocity distribution!')
