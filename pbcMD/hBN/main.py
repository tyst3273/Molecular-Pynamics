...
#   dt = 1e-10 #timestep in ps, nondimensionalized below
   tTot = 1 #200 #ps 
   
   rcut = 10 #cut off distance in units of sigma, neighbor sphere
   ...
   for k in range(steps): #run the MD simulation
       ...
       if k+1 == steps:
          print('\t\tvx, vy, vz =\t'+str(max(vels[:,2]))+' '+str(max(vels[:,3]))+' '+str(max(vels[:,4]))+' ')
          print('\t\tx, y, z =\t'+str(max(pos[:,2]))+' '+str(max(pos[:,3]))+' '+str(max(pos[:,4]))+'\n')
       print('\t\tfx, fy, fz =\t'+str(max(fij[:,2]))+' '+str(max(fij[:,3]))+' '+str(max(fij[:,4]))+' ')
   print('\tALL DONE!')
