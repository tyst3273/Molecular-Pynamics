...
def vInit(pos,dist='constant',val=0):
    ...
    if dist == 'constant': #if constant vel across whole structure
        ...
        for i in range(num):
            if pos[i,1] == 1:
                vels[i,2:5] = vels[i,2:5]*np.sqrt(kb*val/mB) #m/s
                cmv = cmv+vels[i,2:5]*mB
                mass = mass+mB
            else:
                vels[i,2:5] = vels[i,2:5]*np.sqrt(kb*val/mN) #m/s
...
def fLJ_SF(pos,num,rcut,vlist,box):
    ...
    """
    vcB = 4*eB*((sB/rcut)**12-(sB/rcut)**6) #potential at cutoff
    dvdrB = -48*eB*((sB/rcut)**13-0.5*(sB/rcut**7)) #slope of potential at cutoff
    fcB = (48*eB/rcut**2*((sB/rcut)**12-0.5*(sB/rcut)**6)) #force at cutoff
    vcN = 4*eN*((sN/rcut)**12-(sN/rcut)**6) #potential at cutoff
    dvdrN = -48*eN*((sN/rcut)**13-0.5*(sN/rcut**7)) #slope of potential at cutoff
    fcN = (48*eN/rcut**2*((sN/rcut)**12-0.5*(sN/rcut)**6)) #force at cutoff
    ...
    for i in range(num):
        ...
        for j in range(numNN):
            ...
            tmpf[j,:] = ((48*eps/rd**2*((sig/rd)**12-0.5*(sig/rd)**6))-fc)*rv
            tmpv[0,j] = (4*eps*((sig/rd)**12-(sig/rd)**6))-vc-(rd-rcut)*dvdr
...
def vVerlet(num,pos,rcut,vels,fij,vlist,dt,box):
    ...
    """
    c = 0.0001/j2eV #convert ps*eV/ang/kg to ang/ps
...
def thermo(k,thermo,vels,vTot):
    ...
    """
    num = len(vels[:,0])
    vels2 = cp.deepcopy(vels)
    vels2[:,2:5] = vels2[:,2:5]*100 #m/s
    ke = 0
    mx = 0
    my = 0
    mz = 0
